...
exports.Tuple = Tuple;
function Dictionary(v, keyType) {
    return runtype(function (xs) {
        if (xs === null || xs === undefined || typeof xs !== 'object')
            throw new ValidationError("Expected a defined non-null value but was " + typeof xs);
        for (var _i = 0, _a = Object.keys(xs); _i < _a.length; _i++) {
            var x = _a[_i];
            // Object.keys is always a string array, check that key is not the string representation of a number
            if (keyType === 'number') {
                if (isNaN(+x))
                    throw new ValidationError("Expected a number value but was " + typeof x);
            }
            if (keyType === 'string') {
                if ('' + (+x) === x)
                    throw new ValidationError("Expected a string value but was " + typeof x);
                exports.String.check(x);
            }
            v.check(xs[x]);
        }
        return xs;
