...
interface Array<R extends Runtype.Core = Runtype.Core> extends Runtype.Common<Static<R>[]> {
	tag: "array"
	element: R
}
...
const Array = <R extends Runtype.Core>(element: R) =>
	Runtype.create<Array<R>>(
		...
		{ tag: "array", element },
	).with(self => ({ asReadonly: () => self as unknown as ArrayReadonly<R> }))
